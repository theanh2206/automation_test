...
def test_search_services_tc44(driver):
    buypakage = BuyPakagePage(driver)
    buypakage.click_buy_pakage()
    buypakage.search_package("Thế Anh Đẹp Zai")
    buypakage.wait_for_result("Không có dữ liệu")
    assert buypakage.is_result_displayed("Không có dữ liệu")
...
def test_create_pakage_flex_OTP_tc48(driver):
    ...
    buypakage.click_button_by_text("Đăng ký", 4)
    buypakage.click_button_by_text("Đăng ký ngay")
    buypakage.click_button_by_text("Xác nhận thanh toán")
    # buypakage.input_otp("000000")
    buypakage.wait_for_result("Xác nhận OTP")
...
def test_click_question_by_text_tc55(driver):
    ...
    buypakage.click_by_text("Nếu thuê bao tại Việt Nam gọi cho tôi khi tôi đang chuyển vùng tại nước ngoài thì người đó có phải trả cước gọi đi quốc tế không?")
    buypakage.click_by_text("Nếu thuê bao tại Việt Nam gọi cho tôi khi tôi đang chuyển vùng tại nước ngoài thì người đó có phải trả cước gọi đi quốc tế không?")
    buypakage.click_by_text("Khi ra nước ngoài, tôi cần làm gì để kiểm soát được dung lượng và cước phí sử dụng dịch vụ CVQT?")
    buypakage.click_by_text("Khi ra nước ngoài, tôi cần làm gì để kiểm soát được dung lượng và cước phí sử dụng dịch vụ CVQT?")
    buypakage.click_by_text("Tôi đã đăng ký dịch vụ CVQT của MobiFone nhưng khi ra nước ngoài tôi không truy cập được data. Tôi phải làm gì?")
    buypakage.click_by_text("Tôi đã đăng ký dịch vụ CVQT của MobiFone nhưng khi ra nước ngoài tôi không truy cập được data. Tôi phải làm gì?")
...
def test_click_list_categories_tc58(driver):
    ...
    buypakage.click_by_text("Chuyển vùng quốc tế", 2)
    # buypakage.click_by_text("Gói bổ sung")
    buypakage.wait_for_result("Cước tạm tính")
    assert buypakage.is_result_displayed("Cước tạm tính")
...
def test_click_sort_price_tc60(driver):
    ...
    buypakage.click_by_text("Giá")
    buypakage.wait_for_result("Chọn chu kỳ gói")
    assert buypakage.is_result_displayed("Chọn chu kỳ gói")
...
def test_click_utilities_tc70(driver):
    ...
    buypakage.click_by_text("Thanh Toán")
    buypakage.click_button_by_text("icBack")
    buypakage.click_by_text("Lịch sử gói cước")
    buypakage.click_button_by_text("arrow left")
    buypakage.click_by_text("Zalo miniApp")
    buypakage.wait_for_text("Zalo")
    assert buypakage.is_result_displayed("Zalo")